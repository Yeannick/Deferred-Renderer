...
void DeferredLightRenderer::DrawVolumetricLight(const SceneContext& sceneContext, const Light& light) const
{
	...
	const auto pDeviceConext = sceneContext.d3dContext.pDeviceContext;
	auto& techContext = m_pVolumetricLightMaterial->GetTechniqueContext();

	// Set Input Layout
	pDeviceConext->IASetInputLayout(techContext.pInputLayout);

	const UINT offset = 0;
	pDeviceConext->IASetVertexBuffers(0, 1, light.type == LightType::Point ? &m_pSphereVB : &m_pConeVB, &m_VertexStride, &offset);

	pDeviceConext->IASetIndexBuffer(light.type == LightType::Point ? m_pSphereIB : m_pConeIB, DXGI_FORMAT_R32_UINT, 0);

	pDeviceConext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	{
		techContext.pTechnique->GetPassByIndex(p)->Apply(0, pDeviceConext);
		pDeviceConext->DrawIndexed(light.type == LightType::Point ? m_pSphereMesh->GetIndexCount() : m_pConeMesh->GetIndexCount(),0,0);
	}
}
...
void DeferredLightRenderer::CreateReadOnlyDSV(const D3D11Context& d3dContext, ID3D11Resource* pDepthResource, DXGI_FORMAT format)
{
	...
	descDSV.Flags = D3D11_DSV_READ_ONLY_DEPTH; // Depth is Read only , stencil is still read/write
