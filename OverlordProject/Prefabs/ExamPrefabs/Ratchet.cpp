...
#include "Materials/Deffered_Skinned.h"
...
{
	SoundManager::Get()->GetSystem()->createStream("Resources/Audio/Hit.wav", FMOD_DEFAULT, nullptr, &m_pHitSound);
	SoundManager::Get()->GetSystem()->createStream("Resources/Audio/Flip.wav", FMOD_DEFAULT, nullptr, &m_pJumpSound);
...
void Ratchet::Animation()
{
	...
	case Flip:
		...
		m_pAnimator->SetAnimationSpeed(0.75f);
		break;
	case Attacking:
...
void Ratchet::DoubleJump(const SceneContext& )
{
...
void Ratchet::Initialize(const SceneContext&)
{
	...
	m_SpawnPosition = { 80.f, 10.f, -10.f };
	...
	const auto pMaterial = MaterialManager::Get()->CreateMaterial<Deffered_Skinned>();
	pMaterial->SetDiffuseMap(L"Textures/Ratchet/Texture0.png");
	pMaterial->SetSpecularMap(L"Textures/Lava/Specular.png");
	const auto pMaterial1 = MaterialManager::Get()->CreateMaterial<Deffered_Skinned>();
	pMaterial1->SetDiffuseMap(L"Textures/Ratchet/Texture1.png");
	const auto pMaterial2 = MaterialManager::Get()->CreateMaterial<Deffered_Skinned>();
	pMaterial2->SetDiffuseMap(L"Textures/Ratchet/Texture2.png");
	const auto pMaterial3 = MaterialManager::Get()->CreateMaterial<Deffered_Skinned>();
	pMaterial3->SetDiffuseMap(L"Textures/Ratchet/Texture3.png");
	...
	model->GetTransform()->Translate(m_pControllerComponent->GetFootPosition());
...
void Ratchet::Update(const SceneContext& sceneContext)
{
	...
	if (m_pCameraComponent->IsActive())
	{
		...
		if (!m_IsAttacking)
		{
			if (sceneContext.pInput->IsActionTriggered((int)m_CharacterDesc.action_id_Attack))
			{
				m_IsAttacking = true;
				SoundManager::Get()->GetSystem()->playSound(m_pHitSound, nullptr, false, nullptr);
			}
		}
		...
		//m_pCameraComponent->GetTransform()->Rotate(0, m_TotalPitch, 0);
		...
		if (  sceneContext.pInput->IsActionTriggered((int)m_CharacterDesc.action_id_Jump))
		{
			if (m_JumpCounter <= 1)
			{
				Jump(sceneContext);
 		...
 		/*else if (m_State == AnimationState::Jumping && sceneContext.pInput->IsActionTriggered((int)m_CharacterDesc.action_id_Jump))
		{
...
void Ratchet::Draw(const SceneContext& )
{
...
void Ratchet::Attack(const SceneContext& sceneContext)
{
	...
	if (IsInCollider)
	{
		if (m_pCollisionObject)
		{
			...
			if (m_pCollisionObject->GetTag() == L"Enemy")
			{
				auto pEnemy = static_cast<Enemy*>(m_pCollisionObject);
				if (!pEnemy->IsHit())
				{
					pEnemy->Destroy();
				}
...
void Ratchet::OnTriggerCallBack(GameObject* , GameObject* pOtherObject, PxTriggerAction action)
{
	if (pOtherObject)
	{
		...
		if (pOtherObject->GetTag() == L"Enemy" && action == PxTriggerAction::ENTER)
		{
			IsInCollider = true;
			m_pCollisionObject = pOtherObject;

		}
		if (pOtherObject->GetTag() == L"Enemy" && action == PxTriggerAction::LEAVE)
		{
			IsInCollider = false;
			m_pCollisionObject = nullptr;
		}
...
void Ratchet::Reset()
{
	m_Bolts = 0;
	m_PlayerLives = 5;
	GetTransform()->Translate(80, 20, 0);
}
