...
void BoltPickUp::Initialize(const SceneContext&)
{
	...
	const auto pBottomMaterial = MaterialManager::Get()->CreateMaterial<DeferredShadow>();
	const auto pTopMaterial = MaterialManager::Get()->CreateMaterial<DeferredShadow>();

	pBottomMaterial->SetDiffuseMap(L"Textures/BoltBot.png");
	pTopMaterial->SetDiffuseMap(L"Textures/BoltTop.png");
